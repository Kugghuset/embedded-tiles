...
/**
 * @param {Array} _tiles The tiles to search through
 * @param {Object} tile The tile to search for
 * @return {Object}
 */
const _tileExists = (_tiles, tile) => {
  return _.find(_tiles, (t) => {
    // Handle strings
    if (_.isString(t)) {
      return t === tile.id || t === tile.title;
    }

    // Handle objects
    if (_.isObject(t)) {
      return t.title === tile.title || t.id === tile.id;
    }
  });
}
  ...
  .then((tiles) => {
    ...
    resolve(
      ...
      // If _tiles is undefined, filter out none, otherwise filter out all non-matching tiles
        .filter((tile) => {
          return!!_tiles
            ? _tileExists(_tiles, tile)
            : true;
        })
      .map((tile) => {
        let _tile = _tileExists(_tiles, tile);
        return _.assign({}, _tile, { title: tile.title, embedUrl: tile.embedUrl });
      })
      .value()
    );
